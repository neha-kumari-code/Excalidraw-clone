...
export default function editing(ctx:CanvasRenderingContext2D,x:number,y:number,shapes: ShapesType[]){
        ...
        const s=shapes[i];
         ...
         if(s.type===ShapeType.RECT){
            ...
            if(Line(fromX,fromY,toX,toY,x,y)){
                ...
                ctx.font = `${s.data.font ?? 20}px sans-serif`;
                const metrics=ctx.measureText(text);
                const textWidth=metrics.width;
                const textHeight=s.data.font;
                if(textWidth>0 && Rectangle(x,y,s.data.x,s.data.y,textWidth,textHeight)){
                    console.log("yes text is true")
                    return s;
                }
            }
    }
